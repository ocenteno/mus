...
public class TestDaoMazoHibernate {
	...
	public void testInsertarMazoVacio() {
		...
		Assert.assertNotEquals("Debería devolver un ID distinto de cero", 0, m.getId());
		// 4º REPARAR
		dmh.eliminar(m);
	}
	...
	/**
	 * Se inserta un mazo, se modifica su nombre y se comprueba que se cambiado el
	 * nombre en la BD
	 */
	@Test
	public void testActualizar() {
		...
		// 3º VERIFICAR (ASERCIÓN)
		Assert.assertEquals("Debería devolver un mazo con nombre barajaFrancesa", m, enDB);
		// 4º REPARAR
		dmh.eliminar(m);
	}
	 ...
	 * no se recupera el id guardado antes de eliminarlo, sino que se recupera null
	 * de la DB
	 */
	@Test
	public void testEliminar() {
		...
		Mazo enDB = dmh.buscar(m.getId());
		// 3º VERIFICAR (ASERCIÓN)
		Assert.assertNotEquals("Debería devolver null ya que el mazo se ha borrado", guardado, enDB);
	}
	...
	public void testBuscar() {
		// 1º PREPARAR
		Mazo mazo = new Mazo("barajaEspañola");
		dmh.insertar(mazo);
		// 2º TEST
		Mazo m1 = dmh.buscar(mazo.getId());
		// 3º VERIFICAR (ASERCIÓN)
		Assert.assertNotNull("Debería existir un mazo", m1);
		// 4º REPARAR
		dmh.eliminar(mazo);
		// 5º TEST
		Mazo m2 = dmh.buscar(mazo.getId());
		// 6º VERIFICAR (ASERCIÓN)
		Assert.assertNull("El mazo debería haber desaparecido", m2);
	}
}
